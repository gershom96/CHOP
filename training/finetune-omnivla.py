...
# ==============================
from policy_sources.omnivla.experiments.robot.openvla_utils import (
...
from policy_sources.omnivla.prismatic.models.action_heads import L1RegressionActionHead_idcat
from policy_sources.omnivla.prismatic.models.backbones.llm.prompting import PurePromptBuilder
from policy_sources.omnivla.prismatic.models.film_vit_wrapper import FiLMedPrismaticVisionBackbone
from policy_sources.omnivla.prismatic.models.projectors import NoisyActionProjector, ProprioProjector
from policy_sources.omnivla.prismatic.training.train_utils import get_current_action_mask, get_next_actions_mask

from policy_sources.omnivla.prismatic.util.data_utils import PaddedCollatorForActionPrediction_CHOP
from policy_sources.omnivla.prismatic.vla.action_tokenizer import ActionTokenizer
from policy_sources.omnivla.prismatic.vla.constants import ACTION_DIM, NUM_ACTIONS_CHUNK, POSE_DIM, IGNORE_INDEX
...
def run_forward_pass(
    ...
    #batch size
    Bsize = batch["pixel_values"].size()[0]
    
    # Get ground-truth action labels    
    ground_truth_actions = batch["actions"].to(device_id).to(torch.bfloat16)
    modality_id = batch["goal_mask_select"]

    neg_actions = batch["neg_actions"].to(device_id).to(torch.bfloat16)
...
def train_omnivla(cfg: OmniVLAConfig) -> None:
    ...
    if "openvla/openvla-7b" in cfg.vla_path: #from OpenVLA checkpoints
        cfg.resume = False
        cfg.resume_step = None
    elif "omnivla-original" in cfg.vla_path: #from OmniVLA checkpoints (paper version)
        cfg.resume = True     
        cfg.resume_step = 120000 
    elif "omnivla-original-balance" in cfg.vla_path: #from OmniVLA checkpoints (fix LeLaN data unbalance)
        cfg.resume = True     
        cfg.resume_step = 285000  
    elif "omnivla-finetuned-cast" in cfg.vla_path: #from OmniVLA checkpoints fituned with CAST dataset 
        cfg.resume = True      
    ...
    data_config_sub = config["datasets_scand_a"]
    if "waypoint_spacing" not in data_config_sub:
        data_config_sub["waypoint_spacing"] = 1   
    # print(data_config_sub)
    for data_split_type in ["train", "test"]:   
        dataset_chop = OmniVLAChopDataset(
            image_root=data_config_sub["image_root"],
            dataset_path=data_config_sub[data_split_type],
            action_tokenizer=action_tokenizer,
            base_tokenizer=processor.tokenizer, 
            image_transform=processor.image_processor.apply_transform,
            data_split_folder=data_config_sub["data_root"],
        ...
        if data_split_type == "train":
            ...
            test_dataset.append(dataset_chop)

        if data_split_type == "train":
            train_dataset = ConcatDataset(train_dataset)
            sampler_train = DistributedSampler(train_dataset, num_replicas=world_size, rank=device_id, shuffle=True)
            ...
            sampler_test = DistributedSampler(test_dataset, num_replicas=world_size, rank=device_id, shuffle=True)
